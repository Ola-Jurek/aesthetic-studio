...
const toMarkdown = (text) => {
    return text
        .replace(/<\/li>/g, '\n')           // Koniec listy -> nowa linia
        .replace(/<li>/g, '- ')             // Początek listy -> myślnik
        .replace(/<h3>/g, '### ')           // H3 -> Nagłówek MD
        .replace(/<\/h3>/g, '\n')
        .split('\n')                        // ROZBIJAMY NA LINIE
        .map(line => line.trim())           // CZYŚCIMY WCIECIA (to usuwa te taby z lewej!)
        .filter(line => line.length > 0)    // USUwamy puste linie
        .join('\n')                         // ŁĄCZYMY Z POWROTEM
        .replace(/<[^>]*>/g, '')            // Usuwamy resztę tagów HTML
        .trim();
};

console.log(toMarkdown(oldHtml));